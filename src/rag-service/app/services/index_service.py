...
def index_file(documents: list[Document], submission_id: int) -> dict:
    chunks = chunk_llama_documents(documents, submission_id)
    return _embed_and_store(chunks, submission_id)


def index_text(content: str, submission_id: int) -> dict:
    chunks = chunk_document(content, submission_id)
    return _embed_and_store(chunks, submission_id)


def _embed_and_store(chunks: list[dict], submission_id: int) -> dict:
    if not chunks:
